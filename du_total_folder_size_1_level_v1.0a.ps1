# This script will give the total size of each folder and go 1 level down
...
function GetFolderSize($path){
    $total = (Get-ChildItem $path -ErrorAction SilentlyContinue -Recurse -filter $filter | Measure-Object -Property length -Sum -ErrorAction SilentlyContinue).Sum
...
$dirs = Get-ChildItem $dirRoot -ErrorAction SilentlyContinue | Where-Object { $_.psIsContainer}
...
$results | Select-Object TotalSize, Folder | Format-Table -autosize -wrap 

$results | Export-Csv -Path "du_total_folder.csv" -notypeinformation 
